...
namespace KotoriCore.Database.DocumentDb
{
    partial class DocumentDb
    {
        async Task<CommandResult<string>> HandleAsync(PartiallyUpdateDocument command)
        {
            ...
            var document = await FindDocumentByIdAsync(command.Instance, projectUri, command.DocumentId.ToKotoriUri(docType == Enums.DocumentType.Content ? Router.IdentifierType.Document : Router.IdentifierType.Data), null);
            ...
            if (docType == Enums.DocumentType.Content)
            {
                ...
                if (docType == null)
                    throw new KotoriDocumentTypeException(command.DocumentId, "Document type not found.") { StatusCode = System.Net.HttpStatusCode.NotFound };
                ...
                var newDocument = new Markdown(command.DocumentId, command.Content, transformation);
                var newDocumentResult = await newDocument.ProcessAsync();

                var oldDocument = new Markdown(command.DocumentId, Markdown.ConstructDocument(document.Meta, document.Content), null);
                var oldDocumentResult = await oldDocument.ProcessAsync();

                var slug = await FindDocumentBySlugAsync(command.Instance, projectUri, newDocumentResult.Slug, command.DocumentId.ToKotoriUri(Router.IdentifierType.Document));

                if (slug != null)
                    throw new KotoriDocumentException(command.DocumentId, $"Slug '{newDocumentResult.Slug}' is already being used for another document.");
                ...
                var dr = new Markdown(command.DocumentId, Markdown.ConstructDocument(document.Meta, document.Content), null);
            ...
            if (docType == Enums.DocumentType.Data)
            {
                ...
                var newDataM = new Markdown(command.DocumentId, command.Content, transformation);
                var newDataR = await newDataM.ProcessAsync();
                var newData = new Data(command.DocumentId, "[" + JsonConvert.SerializeObject(newDataR.Meta) + "]");
                var newDocuments = newData.GetDocuments();

                if (!idx.HasValue)
                {
                    throw new KotoriDocumentException(command.DocumentId, $"When updating data you need to provide an index.");
                }

                if (idx.HasValue &&
                   newDocuments.Count != 1)
                {
                    throw new KotoriDocumentException(command.DocumentId, $"When updating data at a particular index you can provide just one document only.");
                }
                ...
                if (meta == null ||
                    !meta.Keys.Any())
                    throw new KotoriDocumentException(command.DocumentId, $"The result data document contains no meta after combination. Cannot update.");
